<?php
    ...
    if(isset($_POST['submit'])){
        ...
        $gambar_new = "$Bus.$extensi";
        $tmp = $_FILES['gambar']['tmp_name'];

        if(move_uploaded_file($tmp, 'Bus_img/'.$gambar_new)){
            $sql = "INSERT INTO bus VALUES ('', '$Bus', '$Daerah', '$Tujuan','$Harga', '$gambar_new')";
    ...
    <div class="container">
            <h1>Tambah Bus</h1>
                <form action="" method="POST" class="login-email" enctype="multipart/form-data">
                    <label>Bus</label>
                    <br>
                    <input type="text" placeholder="Nama Bus" name="bus" require>
                    <br>
                    <label>Daerah Terminal</label>
                    <br>
